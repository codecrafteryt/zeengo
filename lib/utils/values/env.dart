...
class Env {
  ...
  static String get yandexMapsApiKey => dotenv.env['YANDEX_MAPS_API_KEY']?.trim() ?? '';
  static String get stripePublishableKey => dotenv.env['STRIPE_PUBLISHABLE_KEY']?.trim() ?? '';
  /// Backend endpoint that returns `{ "client_secret": "..." }` for PaymentIntent.
  static String get stripePaymentIntentUrl => dotenv.env['STRIPE_PAYMENT_INTENT_URL']?.trim() ?? '';
}
